...
레이어드 아키텍처에서 모든 의존 방향이 **도메인을 향하게** 만드는 게 핵심입니다. Repository 인터페이스는 도메인에, 구현체는 인프라에 두어 의존성을 역전시키면(DIP), DB 교체나 비즈니스 변경의 영향이 최소화되고 테스트 시 Fake 구현으로 도메인만 독립 검증할 수 있어요.
