...
순번이 안 보이면 유저는 새로고침하거나 이탈합니다. **Polling**(클라이언트가 2초마다 "내 순번?"을 되묻기)은 단순하지만 대기 인원이 많으면 그 질문 자체가 부하예요. **SSE(Server-Sent Events)**(서버가 순번이 바뀔 때만 한 방향으로 밀어주는 방식)는 불필요한 요청이 없지만, 대기 인원만큼 연결을 계속 열어둬야 합니다. Polling으로 시작해 부하가 문제되면 SSE로 전환하는 걸 권해요. 그리고 "약 N분 남았습니다"가 순번 숫자보다 이탈 방지에 효과적입니다.
